...
## 4. Paleta y tipografía ya en uso (para que el logo nazca coherente, no separado)

Estos valores ya están validados en el sitio y en la guía de producción de contenido (\`guia-produccion.md\` v2 §2.2–2.3) — el logo tiene que nacer de esta paleta, no definir una nueva:

| Uso | Color | Código |
|---|---|---|
| Primario / fondo de marca | Verde botella | \`#1F4A38\` |
| Fondo secundario / crema | Crema (parchment) | \`#F1EBDD\` |
| Texto / tinta | Marrón oscuro | \`#241B14\` |
| Acento | Naranja ember | \`#F0A04B\` |

**Tipografía:**
- Display / títulos: **Anton** o **Bebas Neue** (condensada, fuerte — la familia "Big Shoulders" del sitio vive en este mismo espíritu).
- Cuerpo: **Schibsted Grotesk** (o Poppins como alternativa).
- Datos / números: **IBM Plex Mono**.
...
silhouette, no gradients, no photorealism, no 3D, no drop shadows.

Color palette (use exactly these): deep bottle green #1F4A38 as the
primary/background color, warm cream #F1EBDD as the secondary/paper
color, dark ink brown #241B14 for line work or text, warm ember orange
#F0A04B as a single accent color used sparingly (one shape or one detail
only, not the whole mark).

Concept direction (pick ONE per generation, don't blend):
1. A stamped order ticket / comanda: a simple ticket or stub shape with
   a torn or perforated edge, suggesting a kitchen order slip, rendered
   as a clean geometric icon rather than a literal illustration.
2. A single check-mark or arrow built from three stacked bars,
   abstracting the live order board (new → kitchen → done) into one
   mark without looking like a generic Kanban/dashboard icon.
...
all lowercase or all caps, tight letter-spacing, sitting confidently
next to the icon — not a thin, delicate, "startup" typeface.

Mood: like a well-made stamp or a hand-painted sign on a neighborhood
...
background texture, no mockup, no phone frame).

Avoid: QR code motifs, generic phone-with-checkmark icons, fork-and-
...
Una vez elegido un concepto que funcione, pedir estas variantes (mismo prompt base + la aclaración entre corchetes):

1. **[Add: "flat version using only #1F4A38 and #F1EBDD, no ember accent"]** — para usos en un solo color (sello, grabado, blanco y negro).
2. **[Add: "reversed version: cream background #F1EBDD, bottle green #1F4A38 icon"]** — para fondos claros (el sitio ya usa fondo crema en varias secciones).
3. **[Add: "icon only, no text, square, for use as an app icon and Instagram profile photo"]** — versión sin wordmark, la que va a vivir como ícono de PWA y foto de perfil.
