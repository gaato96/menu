...
/** Looks up a business by its public slug. 404s if it does not exist or is not servable. */
export async function getPublicMenu(slug: string): Promise<PublicMenuData> {
  ...
  const [settingsResult, hoursResult, categoriesResult, productsResult, groupsResult, optionsResult, zonesResult] =
    await Promise.all([
...
export async function loadMenuSnapshotForPricing(businessId: string): Promise<MenuSnapshot> {
  ...
  const [settingsResult, productsResult, groupsResult, optionsResult, zonesResult] = await Promise.all([
